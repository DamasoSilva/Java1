...
<head>
    <meta charset="UTF-8">
    <title>Meus Agendamentos - Profissional - Salão de Beleza</title>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <c:if test="${agendamento.status == 'PENDENTE'}">
                                                <form action="${pageContext.request.contextPath}/profissional/agendamento/confirmar/${agendamento.id}" method="post" style="display:inline;" onsubmit="return confirm('Confirmar este agendamento?');">
                                                    <button type="submit" class="btn btn-success btn-sm">Confirmar</button>
                                                </form>
                                            </c:if>
                                            <c:if test="${agendamento.status == 'PENDENTE' || agendamento.status == 'CONFIRMADO'}">
                                                <form action="${pageContext.request.contextPath}/profissional/agendamento/cancelar/${agendamento.id}" method="post" style="display:inline;" onsubmit="return confirm('Cancelar este agendamento?');">
                                                    <button type="submit" class="btn btn-danger btn-sm">Cancelar</button>
                                                </form>
                                            </c:if>
                                            <c:if test="${agendamento.status == 'CONFIRMADO'}">
                                                <form action="${pageContext.request.contextPath}/profissional/agendamento/concluir/${agendamento.id}" method="post" style="display:inline;" onsubmit="return confirm('Concluir este agendamento?');">
